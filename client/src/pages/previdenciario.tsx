...
function naturezaLabel(n: NaturezaVinculo) {
  const m: Record<string, string> = {
    ...
    BENEFICIO_INCAPACIDADE: "BenefÃ­cio Incapacidade",
...
export default function PrevidenciarioPage() {
  ...
  const processarPdf = async (file: File, tipo: "cnis" | "carta") => {
    ...
    try {
      ...
      toast({ title: `${tipo === "cnis" ? "CNIS" : "Carta"} importado com sucesso`, description: `Dados extraÃ­dos pelo Gemini AI.` });
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <header className="sticky top-0 z-20 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">CNIS + Carta de ConcessÃ£o via Gemini AI</p>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <UploadZone
            label="Importar CNIS"
            sublabel="PDF do Extrato PrevidenciÃ¡rio (Meu INSS)"
            icon={FileText}
            status={cnisStatus}
          ...
          />
          <UploadZone
            label="Importar Carta de ConcessÃ£o"
            sublabel="PDF da Carta de BenefÃ­cio do INSS"
            icon={FileUp}
            status={cartaStatus}
        ...
        {!temDados && (
          <div className="text-center py-16 text-gray-400 dark:text-gray-600">
            ...
            <p className="text-xs mt-1">O Gemini AI reconhece automaticamente o padrÃ£o de cada documento</p>
          </div>
        )}
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-gray-200 dark:border-gray-700">
                        ...
                        <th className="text-left py-2 pr-4 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">InÃ­cio</th>
                        <th className="text-left py-2 pr-4 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Fim</th>
                        <th className="text-left py-2 pr-4 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Empresa / DescriÃ§Ã£o</th>
                        <th className="text-left py-2 pr-4 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Natureza</th>
                        <th className="text-left py-2 font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">CarÃªncia</th>
                      </tr>
                    </thead>
                    <tbody>
        ...
        {cartaData && (
          <div className="space-y-4">
            ...
            <SectionCard title="Dados do BenefÃ­cio" icon={Hash}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                <InfoRow label="NÃºmero do BenefÃ­cio" value={cartaData.numeroBeneficio} />
                <InfoRow label="EspÃ©cie" value={cartaData.especie} />
                <InfoRow label="CÃ³digo" value={cartaData.codigoEspecie} />
                <InfoRow label="DIB (InÃ­cio BenefÃ­cio)" value={cartaData.dib} />
                <InfoRow label="DIP (InÃ­cio Pagamento)" value={cartaData.dip} />
            ...
            {cnisData && cartaData && (
              <div className="rounded-xl border border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/30 p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-sm text-amber-800 dark:text-amber-300">Comparativo CNIS Ã Carta</h3>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                  <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-amber-200 dark:border-amber-700">
                    ...
                    <p className="font-bold text-gray-800 dark:text-gray-200 text-xs">{cnisData.dadosSegurado?.nome || "â"}</p>
                  </div>
                  <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-amber-200 dark:border-amber-700">
                    <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">TC (Carta)</p>
                    <p className="font-bold text-gray-800 dark:text-gray-200 text-xs">{cartaData.tempoContribuicao || "â"}</p>
                  </div>
                  <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-amber-200 dark:border-amber-700">
                    <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">PerÃ­odos no CNIS</p>
                    <p className="font-bold text-gray-800 dark:text-gray-200 text-xs">{cnisData.periodosContribuicao?.length || 0} vÃ­nculos</p>
                  </div>
                </div>
              </div>
            )}
